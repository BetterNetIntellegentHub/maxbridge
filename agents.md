...
6. После успешного `docker push` и/или deploy выполнять безопасную очистку неиспользуемых артефактов:
   ...
   - на сервере: только prune неиспользуемых Docker cache/images/stopped containers без остановки активных контейнеров;
   - не удалять данные, используемые running-сервисами (active containers/volumes/images in use).
   - после очистки фиксировать результат проверками: `docker system df`, свободное место на диске хоста, `docker ps` на сервере.
   - если после очистки Docker/Go/%TEMP% место на `C:` не возвращается, проверить и отдельно планировать compact `*.vhdx` (WSL/Docker); compact не выполнять автоматически без явного подтверждения пользователя.
...
4. Выполнить безопасную post-task очистку локальных и серверных временных/кеш-артефактов (если они создавались в ходе работы), не затрагивая Go build/module cache без явного запроса пользователя.
5. Явно указать ограничения верификации (если нет runtime/инструментов).
