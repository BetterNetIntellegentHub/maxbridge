...
4. Runner model:
   - build/image jobs могут идти на shared Docker runner.
   - deploy/rollback jobs идут только на self-hosted WSL runner tag `wsl-deploy`.
   - deploy availability зависит от online-состояния WSL runner.
5. Ansible deploy path in jobs:
   - sync compose/env;
